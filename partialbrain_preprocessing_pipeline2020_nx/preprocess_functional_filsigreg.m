function filename=preprocess_functional_filsigreg(subjectdir, filename, fil, gsr, wmcsfr)
...
cmd = ['3dBandpass -prefix ' filename '_fil.nii.gz -quiet -overwrite ' num2str(fil(1)) ' ' num2str(fil(2)) ' ' filename '.nii.gz'];
% cmd = ['3dBandpass -prefix ' filename '_temp_tf -quiet -overwrite ' num2str(fil(1)) ' ' num2str(fil(2)) ' ' filename '.nii.gz'];
system(cmd);
...
T = datetime('now'); time = whatsthetime(T);
fprintf([time,sub, scan ': ','Functional WM/CSF signal regression ... ']) %revised by NX 06262020
if wmcsfr == 1
    if gsr == 0
        nii = load_untouch_nii([filename,'.nii.gz']);
        f = nii.img;
